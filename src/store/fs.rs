...
use std::fs;
use crate::crypto::aeadx::{seal_xchacha, open_xchacha, KEY_LEN, rand_nonce};
use argon2::{Argon2, Algorithm, Params, Version};
use argon2::password_hash::SaltString;
use rand::rngs::OsRng;
use zeroize::Zeroize;

pub struct FileStore { dir: PathBuf, key: [u8; KEY_LEN] }
impl FileStore {
    ...
    pub fn open(dir: PathBuf, passphrase: &str, profile: ArgonProfile) -> anyhow::Result<Self> {
        ...
        let salt = if salt_path.exists() { fs::read(&salt_path)? } else {
            let s = SaltString::generate(&mut OsRng); fs::write(&salt_path, s.as_str().as_bytes())?; s.as_str().as_bytes().to_vec()
        };
        let mut key = [0u8; KEY_LEN];
        derive_key_argon2id(passphrase.as_bytes(), &salt, profile, &mut key)?;
        Ok(Self { dir, key })
    }
    fn path(&self, name: &str) -> PathBuf { self.dir.join(format!("{name}.blob")) }
}
fn derive_key_argon2id(pass: &[u8], salt: &[u8], profile: ArgonProfile, out: &mut [u8; KEY_LEN]) -> anyhow::Result<()> {
    ...
    let params = Params::new(m_cost, t_cost, p_cost, Some(out.len())).map_err(|e| anyhow::anyhow!("argon2 params: {:?}", e))?;
    let argon2 = Argon2::new(Algorithm::Argon2id, Version::V0x13, params);
    argon2.hash_password_into(pass, salt, out).map_err(|e| anyhow::anyhow!("argon2 hash: {:?}", e))?;
    Ok(())
}
#[derive(Clone, Copy)]
pub enum ArgonProfile { FAST, BALANCED, STRONG }
...
impl StateStore for FileStore {
    fn save<T: Serialize>(&mut self, name: &str, value: &T) -> anyhow::Result<()> {
        ...
        blob.extend_from_slice(&u32::try_from(nonce.len()).expect("nonce fits u32").to_le_bytes());
    ...
    fn load<T: DeserializeOwned>(&mut self, name: &str) -> anyhow::Result<Option<T>> {
        ...
        if blob.len() < 4 { anyhow::bail!("corrupt blob"); }
        let nlen = u32::from_le_bytes(blob[0..4].try_into().unwrap()) as usize;
        let nonce = &blob[4..4+nlen];
        let ct = &blob[4+nlen..];
        let ad = name.as_bytes();
        let pt = open_xchacha(&self.key, nonce.try_into().unwrap(), ct, ad).ok_or_else(|| anyhow::anyhow!("decrypt"))?;
