...
fn le_u32(x: u32) -> [u8;4] { x.to_le_bytes() }
fn le_u64(x: u64) -> [u8;8] { x.to_le_bytes() }

fn encode_leaf(l: &Leaf) -> Vec<u8> {
    let mut v = Vec::new();
    v.extend_from_slice(&le_u32(u32::try_from(l.user_id.len()).expect("len fits u32"))); v.extend_from_slice(l.user_id.as_bytes());
    v.extend_from_slice(&le_u32(u32::try_from(l.device_id.len()).expect("len fits u32"))); v.extend_from_slice(l.device_id.as_bytes());
    v.extend_from_slice(&le_u32(u32::try_from(l.pk.len()).expect("len fits u32"))); v.extend_from_slice(&l.pk);
...
#[must_use]
pub fn verify_inclusion(root: &[u8;32], leaf_hash: &[u8;32], idx: usize, proof: &[[u8;32]]) -> bool {
    let mut acc = *leaf_hash;
    let mut i = idx;
    for s in proof {
        acc = if i % 2 == 0 { hash_node(&acc, s) } else { hash_node(s, &acc) };
        i /= 2;
    }
    &acc == root
}
...
pub fn sign_sth(sk: &SigningKey, tree_size: u64, root: [u8;32], timestamp_ms: u64) -> Sth {
    let mut s = Sth { tree_size, root, timestamp_ms, sig: Vec::new() };
...
pub fn verify_sth(vk: &VerifyingKey, sth: &Sth) -> bool {
    if sth.sig.len() != 64 { return false; }
    let m = sth_to_bytes(sth);
    let arr: [u8;64] = match sth.sig.clone().try_into() { Ok(a) => a, Err(_) => return false };
