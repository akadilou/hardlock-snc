use crate::crypto::aeadx::{seal_xchacha, open_xchacha, rand_nonce, KEY_LEN, XNONCE_LEN};
use hkdf::Hkdf;
use sha2::Sha256;
...
pub fn derive_k_s(master: &[u8], salt: &[u8]) -> [u8; KEY_LEN] {
    ...
    hk.expand(b"hardlock/sealed-sender/kS", &mut k).expect("hkdf");
    k
}

pub fn token_build(k_s: &[u8; KEY_LEN], expiry_unix_s: u64, sender_pub32: &[u8;32], scope: &[u8]) -> SenderToken {
...
pub fn token_verify(k_s: &[u8; KEY_LEN], token: &SenderToken, now_unix_s: u64) -> Option<(u64, [u8;32], Vec<u8>)> {
    let ad = b"hardlock/sealed-sender";
    let pt = open_xchacha(k_s, &token.nonce, &token.ct, ad)?;
    if pt.len() < 8 + 32 + 2 { return None; }
    let mut off = 0usize;
    let mut tsb = [0u8;8]; tsb.copy_from_slice(&pt[off..off+8]); off += 8;
    let expiry = u64::from_le_bytes(tsb);
    if now_unix_s > expiry { return None; }
    let mut pk = [0u8;32]; pk.copy_from_slice(&pt[off..off+32]); off += 32;
    let mut slb = [0u8;2]; slb.copy_from_slice(&pt[off..off+2]); off += 2;
    let sl = u16::from_le_bytes(slb) as usize;
    if pt.len() < off + sl { return None; }
    let scope = pt[off..off+sl].to_vec();
    Some((expiry, pk, scope))
}

#[derive(Clone, Copy)]
pub enum PadProfile { Stealth, Balanced, Throughput }
