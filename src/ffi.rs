...
#[no_mangle]
pub extern "C" fn hardlock_consts_header_len() -> usize {
    HEADER_LEN
}

#[no_mangle]
pub extern "C" fn hardlock_consts_nonce_len() -> usize {
    XNONCE_LEN
}
...
#[no_mangle]
pub extern "C" fn hardlock_hpke_initiate(
    ...
    let (enc, okm) = match hpke_initiate(&pk) {
        Ok(v) => v,
        Err(_) => return -2,
    };
    if enc.len() > enc_cap {
...
#[no_mangle]
pub extern "C" fn hardlock_hpke_accept(
    ...
    let okm = match hpke_accept(&sk, enc) {
        Ok(v) => v,
        Err(_) => return -2,
    };
    unsafe {
...
#[no_mangle]
pub extern "C" fn hardlock_ratchet_new_initiator(
    okm32: *const u8,
...
#[no_mangle]
pub extern "C" fn hardlock_ratchet_new_responder(
    okm32: *const u8,
    dh_s_priv32: *const u8,
    dh_r_pub32: *const u8,
) -> *mut RatchetHandle {
...
#[no_mangle]
pub extern "C" fn hardlock_ratchet_encrypt(
    h: *mut RatchetHandle,
    ad_ptr: *const u8,
    ad_len: usize,
    pt_ptr: *const u8,
    ...
    let ad = if ad_ptr.is_null() {
        ...
        unsafe { std::slice::from_raw_parts(ad_ptr, ad_len) }
    };
...
#[no_mangle]
pub extern "C" fn hardlock_ratchet_decrypt(
    h: *mut RatchetHandle,
    ad_ptr: *const u8,
    ad_len: usize,
    header_ptr: *const u8,
    ...
    let pt = match ratchet::decrypt(st, ad, &hdr, &nonce, ct) {
        Ok(v) => v,
        Err(_) => return -3,
    };
    if pt.len() > pt_cap {
