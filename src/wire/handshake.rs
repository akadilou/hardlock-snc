...
pub fn decode_init(b: &[u8]) -> anyhow::Result<Vec<u8>> {
    if b.len() < 5 { anyhow::bail!("short"); }
    if b[0] != TYPE_INIT { anyhow::bail!("bad type"); }
    let len = u32::from_le_bytes(b[1..5].try_into().unwrap()) as usize;
    if b.len() < 5 + len { anyhow::bail!("truncated"); }
    Ok(b[5..5+len].to_vec())
}
...
pub fn decode_init_v2(b: &[u8]) -> anyhow::Result<(u8, Vec<u8>, [u8;32])> {
    if b.len() < 1+1+4+32 { anyhow::bail!("short"); }
    if b[0] != TYPE_INIT { anyhow::bail!("bad type"); }
    let suite = b[1];
    let len = u32::from_le_bytes(b[2..6].try_into().unwrap()) as usize;
    if b.len() < 6 + len + 32 { anyhow::bail!("truncated"); }
...
pub fn decode_init_auth(b: &[u8]) -> anyhow::Result<Vec<u8>> {
    if b.len() < 5 { anyhow::bail!("short"); }
    if b[0] != TYPE_INIT_AUTH { anyhow::bail!("bad type"); }
    let len = u32::from_le_bytes(b[1..5].try_into().unwrap()) as usize;
    if b.len() < 5 + len { anyhow::bail!("truncated"); }
    Ok(b[5..5+len].to_vec())
}
