...
struct Args { #[arg(short, long, default_value_t = 3)] rounds: usize }
...
fn main() -> anyhow::Result<()> {
    ...
    let mut r_alice = ratchet::init_initiator(s_a, alice.x25519.sk.clone().try_into().unwrap(), bob.x25519.public());
    let mut r_bob   = ratchet::init_responder(s_b, bob.x25519.sk.clone().try_into().unwrap(), alice.x25519.public());
    let ad = b"hardlock/example";
    for i in 0..args.rounds {
        let (h, n, ct) = ratchet::encrypt(&mut r_alice, ad, format!("Hello {} from Alice", i).as_bytes());
        let pt = ratchet::decrypt(&mut r_bob, ad, &h, &n, &ct).map_err(|_| anyhow::anyhow!("decrypt"))?;
        println!("Bob got: {}", String::from_utf8_lossy(&pt));
        let (h2, n2, ct2) = ratchet::encrypt(&mut r_bob, ad, format!("Ack {} from Bob", i).as_bytes());
        let pt2 = ratchet::decrypt(&mut r_alice, ad, &h2, &n2, &ct2).map_err(|_| anyhow::anyhow!("decrypt"))?;
